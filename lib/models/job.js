...
module.exports = {
   ...
   * @param direction {Boolean} asc/desc
   * @returns {Promise} A promise that resolves to an array of jobs
   */
  fetch: (qName, type, offset, limit, direction) => {
    const q = queue.get(qName);
    if (!(offset >= 0)) {
      offset = 0;
    }
    ...
    if (order[type] !== undefined) {
      return q.getJobs(type, offset, offset + limit - 1, direction === undefined ? order[type] : direction)
